...
locals {
  vn_leaf_labels = toset(flatten([
    for vn in local.netbox_vlans : vn.bindings
  ]))
}
...
# One tagged connectivity template is automatically created per virtual network.
...
resource "apstra_datacenter_connectivity_template_interface" "vn_ct" {
  ...
  description = "Tagged interface CT for virtual network ${each.key}"
